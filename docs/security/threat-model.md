...
That defect was found by a fuzz campaign against `pkg/revoke`, a target which had been in the repository for days without being in the nightly matrix. The lesson recorded here is not about the bound but about the coverage: a fuzz target nothing runs is a test suite entry that reads as assurance.
