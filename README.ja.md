...
> **ステータス: 初期。** フォーマット、検証器、署名層、発行器は一通り動き、実際の署名に対してテストされています。AuthZEN バインディングと並び評価も動きます。発行者の JWKS / SPIFFE bundle からの鍵解決と、公開されたコンパクト集合によるローカル失効判定も動きます。監査ログは未実装、並び状態のストアはプロセス内のみ、失効集合を定期的に公開する仕組みも無く、第三者によるセキュリティレビューも受けていません。それが実務上どういう意味かは [ROADMAP.md](ROADMAP.md) と[脅威モデル](docs/security/threat-model.md)（英語）にあります。
>
...
*translated-from: sha-256:937b1dd15819b5436076c00cea87c8b227b8e2edd0e387df43243b67a855da4c*
