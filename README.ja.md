...
**AI エージェントのための検証可能な委任。** エージェントが行為する権限を、実在する人間を根とする署名済みの連鎖として表現します。連鎖は一段ごとに権限が狭まり、任意のリンク単位で失効でき、単発の呼び出しではなく行動の並び全体に対して評価されます。

> **ステータス: 初期。** フォーマット、検証器、署名層、発行器は一通り動き、実際の署名に対してテストされています。AuthZEN バインディングと並び評価も動きます。発行者の JWKS / SPIFFE bundle からの鍵解決、公開されたコンパクト集合によるローカル失効判定、そして仕様が要求する順序で4つの検査を回す MCP ツール呼び出し強制ミドルウェアも動きます。監査ログは未実装、並び状態のストアはプロセス内のみ、失効集合を定期的に公開する仕組みも無く、第三者によるセキュリティレビューも受けていません。それが実務上どういう意味かは [ROADMAP.md](ROADMAP.md) と[脅威モデル](docs/security/threat-model.md)（英語）にあります。
>
...
6 は、強制点が最も省きやすい手順です。どうせ次に PDP が答えるのだから、と。ここでやっているのは呼び出しをスポンサーが実際に与えた権限と突き合わせることであり、これがあるおかげで「何でも許可する PDP」でも誰も委任していないツールを認可できません。9 が最後なのにも理由があります。行為の受理は同時に記録でもあり、PDP が拒否した呼び出しで連鎖の呼び出し予算を使わせるべきではないからです。

13 が他のどこもやっていないものです。どちらの呼び出しも個別には認可されています。その組が情報漏洩であり、1呼び出しずつ見るチェックには判別できません。
...
- **ゲートウェイ・レジストリ・サンドボックス・エージェントランタイムではない。** それらのカテゴリは混雑しています。Mandatum はライブラリであり、agentgateway や ToolHive、MCP サーバの**内側で**動くことを意図しています。`pkg/mcp` がそこに置くための `net/http` ミドルウェアです。`tools/call` を強制し、それ以外のメソッドは前段にいるサーバへそのまま流します。
...
強制する側にもう1つあります。何でも許可する Policy Decision Point のもとでデータベース全消去を要求し、その呼び出しが — PDP に問い合わせることすらなく — 拒否されます。
...
*translated-from: sha-256:04c62abd3138f4bd4477072213a3d624bec8da3e3263ed3b47ea2c7783435b07*
