...
- CI refuses a document that references a document which does not exist, unless it is declared unwritten in the Planned table. The same defect appeared three times, each found by reading; whoever writes a sentence claiming something is recorded in another file intends to write that file, which is why they never notice it is missing. The inverse is checked too: an entry still listed as Planned after the document exists misleads a reader the other way.
