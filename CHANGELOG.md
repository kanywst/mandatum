...
- §8.1 specifies how a chain reaches an enforcement point: an array of compact serializations in the `tools/call` request's `_meta`, under a key MCP's own naming rules mark as third-party. The AuthZEN binding says what a PDP is told and, having no delegation chain of its own, says nothing about how one arrives — and a chain that travels differently in two deployments is one neither deployment's middleware can read.
- §8's ordering gains the sequence step, and says why it is last: admitting an action is also recording it, so a call the PDP refused would otherwise spend a sponsor's invocation budget on something that never ran.
...
- **A published revocation set could make every authorization cost seconds.** `hashes` had a lower bound and no upper one, and a lookup costs one hash position per declared hash function — so eighty bytes declaring *k* in the billions made one `MayContain` take three seconds and ask for fourteen gigabytes, on the path of every authorized action. The document that says which agents are revoked doubled as a way to stop the enforcement point evaluating anything. §7.1 now bounds *k* at 64, which is past any operational need (at optimal sizing *k* = −log₂ *p*, so 64 is a rate near 10⁻¹⁹, and one in a billion is *k* = 30), and `ParseSet` refuses a set outside 1..64. `NewSet` refuses a false-positive rate it cannot build within that bound rather than clamping, because a publisher who believes they got 10⁻³⁰ and got 10⁻¹⁹ is worse off than one who got an error. Found by the nightly fuzz campaign, in the target that had never run one until this release; the input is in the seed corpus.
- **A sub-delegation could re-target the chain at a resource server the sponsor never named.** An agent holding a valid chain signed a further link naming somebody else's server: same capabilities, itself as delegator, so every attenuation rule held, and V9 at the target compared the leaf's audience against the target and passed. Every link valid, every rule satisfied, and the sponsor authorized none of it. §6 gains rule 7 — `aud(i)` = `aud(i-1)` — checked by `verify.Attenuates`, so the issuer refuses to produce such a link and the verifier refuses to accept one. The §11 confused-deputy row and the threat model said V9 and V2 answered this. They did not.
- **Verification evaluated claims the caller supplied rather than the claims the issuer signed.** V3 verifies a signature over `Assertion.Raw`; every other rule read `Assertion.Claims`, a struct field anyone can set, and nothing compared the two. A caller could hand over an untouched signed assertion with a wider capability set in the struct and the whole rule set would run against claims nobody signed — with the correct signature over the real bytes making it look checked. `Verify` now re-derives claims from `Raw` through the new `mda.ParseClaims`, and `mda.Assertion`'s documentation says the field is a convenience rather than an invariant, which is what it always was. The test suite could not have caught this: its chain builder fabricated `Raw` as a stand-in, with a comment saying verification treats it as opaque bytes. `issue.ParseChain` is deprecated in favour of `mda.ParseChain`.
