...
- The alternatives analysis covers the OAuth Actor Profile drafts, which are the closest standards work to a Mandatum chain and had been missed entirely. `draft-mcguinness-oauth-actor-profile` gives a resource server processing rules for identifying the current actor, and says in its own words that it does not provide independent cryptographic provenance per hop; the companion `draft-mcguinness-oauth-actor-proofs` adds that, and still does not prove authority did not widen. It also corrects a claim that `modelcontextprotocol/ext-auth#14` had gone unanswered by maintainers, which was wrong when written.
